...
public class TileDeserializer implements JsonDeserializer<Tile> {
	...
	public Tile deserialize(JsonElement json, Type type, JsonDeserializationContext context) throws JsonParseException {
		...
		Hashtable<String, String> attributes = context.deserialize(j.get("attributes"),
				new TypeToken<Hashtable<String, String>>() {
				}.getType());
		ArrayList<Tile> neighbors = context.deserialize(j.get("neighbors"), new TypeToken<List<String>>() {
		}.getType());
